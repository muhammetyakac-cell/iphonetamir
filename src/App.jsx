...
/**
 * --- KRİTİK AYARLAR ---
 * SHEET_URL: Google Apps Script'ten aldığınız URL.
 */
...
const App = () => {
  ...
  /**
   * GÜVENLİK: Şifreler Vercel ortam değişkenlerinden çekilir.
   * Kodun içinde hiçbir sabit (hardcoded) şifre bırakılmamıştır.
   * Lütfen Vercel panelinden VITE_ADMIN_USER ve VITE_ADMIN_PASS değerlerini tanımlayın.
   */
  const getAdminUser = () => {
    try {
      // @ts-ignore
      return import.meta.env.VITE_ADMIN_USER || "";
    } catch (e) {
      return "";
    }
  };
  ...
  const getAdminPass = () => {
    try {
      // @ts-ignore
      return import.meta.env.VITE_ADMIN_PASS || "";
    } catch (e) {
      return "";
    }
  };

  const ADMIN_USERNAME = getAdminUser();
  const ADMIN_PASSWORD = getAdminPass();
  
  const [adminCreds, setAdminCreds] = useState({ username: '', password: '' });
  const [formData, setFormData] = useState({ name: '', phone: '', area: 'Buca', address: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      name: formData.name, phone: formData.phone, area: formData.area, address: formData.address,
      model: selectedModel, service: selectedService,
  ...
  const handleAdminLogin = (e) => {
    e.preventDefault();
    // Eğer env değişkenleri tanımlı değilse giriş yapılamaz (Güvenli yöntem)
    if (ADMIN_USERNAME && ADMIN_PASSWORD && 
        adminCreds.username === ADMIN_USERNAME && 
        adminCreds.password === ADMIN_PASSWORD) {
      setView('admin');
    } else {
      alert("Giriş bilgileri hatalı veya henüz tanımlanmamış!");
    }
  };

  // --- RENDER: YÖNETİCİ GİRİŞ ---
  if (view === 'login') {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-[40px] w-full max-w-md shadow-2xl">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tight text-slate-800 tracking-widest">GİRİŞ YAP</h2>
          </div>
          <form onSubmit={handleAdminLogin} className="space-y-4">
            ...
            <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-100">Panel Girişi</button>
  ...
  if (view === 'admin') {
    return (
      <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
        ...
        <div className="container mx-auto p-6 max-w-5xl text-slate-900">
          ...
          <div className="grid gap-4 text-slate-900">
            {isLoading && requests.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4 text-slate-900">
                <Loader2 className="animate-spin text-blue-600" size={40} />
                <p className="text-slate-400 font-medium italic">Veriler Excel'den çekiliyor...</p>
              </div>
            ) : requests.length === 0 && !error ? (
              <div className="bg-white p-20 rounded-[40px] text-center border border-slate-200 shadow-sm text-slate-900">
                 <p className="text-slate-400 font-medium italic">Kayıtlı veri bulunamadı.</p>
              </div>
            ) : (
              requests.map((req, idx) => (
                <div key={idx} className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between gap-6 hover:shadow-md transition text-slate-900">
        ...
        <section id="home" className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden">
          ...
          <div className="container mx-auto px-4 flex flex-col md:flex-row items-center">
            <div className="md:w-1/2 space-y-6 text-center md:text-left">
              ...
              <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 leading-tight tracking-tighter uppercase">İzmir iPhone <br /><span className="text-blue-600">Ekran & Batarya</span> Tamiri</h1>
              <p className="text-lg text-slate-600 max-w-lg mx-auto md:mx-0 font-medium leading-relaxed">iPhone'unuzu servis servis gezdirmeyin. İzmir'in her semtinde kapınızdan alıyor, orijinal kalitede parçalarla onarıp aynı gün adresinize teslim ediyoruz. Üstelik 6 ay parça garantisiyle!</p>
               ...
               <div className="relative z-10 animate-float bg-white p-6 rounded-3xl shadow-2xl max-w-sm border border-slate-100">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                        <span className="text-xs text-slate-400 font-bold uppercase tracking-widest text-slate-900">Teknik Durum</span>
                        <p className="text-orange-500 font-black">Onarım Bekliyor</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between text-sm"><span className="text-slate-500 font-medium">Kurye Yolda</span><span className="font-black">14:20 Varış</span></div>
                  </div>
               </div>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-slate-900">
             <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 transition hover:shadow-lg">
               ...
               <h3 className="font-black text-xl mb-2 uppercase tracking-tighter">Ekran Değişimi</h3>
               <p className="text-slate-500 text-sm font-medium leading-relaxed">30 dakikada montaj, True Tone aktarımı ve sıvı koruma bandı ile profesyonel İzmir iPhone ekran değişimi çözümleri.</p>
             </div>
             <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 transition hover:shadow-lg">
               ...
               <h3 className="font-black text-xl mb-2 uppercase tracking-tighter">Batarya Değişimi</h3>
             ...
             <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 transition hover:shadow-lg">
               ...
               <h3 className="font-black text-xl mb-2 uppercase tracking-tighter">Garantili Cam Onarımı</h3>
               <p className="text-slate-500 text-sm font-medium leading-relaxed">Lazer teknolojisi ile kasa değişmeden kusursuz, garantili iPhone arka cam tamiri İzmir genelinde kapınızda.</p>
             </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-4xl text-center text-slate-900">
          <div className="bg-white p-8 rounded-[50px] border border-slate-100 grid md:grid-cols-2 gap-10 items-center">
            <div className="space-y-6 text-left">
              <h2 className="text-3xl font-black tracking-tighter uppercase">2024 iPhone Fiyat Listesi</h2>
              <p className="text-slate-500 text-sm font-medium">Modelinizi seçin, İzmir iPhone ekran ve batarya değişim fiyatlarını anında kurye dahil görün.</p>
            ...
            <div className="bg-blue-600 rounded-[40px] p-10 text-white shadow-xl">
               <p className="text-blue-200 text-xs font-bold uppercase mb-2">Net İşçilik + Kurye Dahil</p>
            ...
            <div className="bg-white p-8 md:p-12 rounded-[50px] shadow-2xl border border-blue-50">
                {formStep === 1 ? (
                    <form onSubmit={handleSubmit} className="space-y-6 text-slate-900">
                        <h2 className="text-3xl font-black text-center mb-8 uppercase text-slate-900 tracking-tighter">İzmir iPhone Başvuru</h2>
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">İletişim Hattı</label>
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Onarım Adresi</label>
                        ...
                        <div className="bg-blue-50/50 p-6 rounded-[32px] space-y-4 border border-blue-100">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1 text-slate-900">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tamir İşlemi</label>
                                <div className="relative">
                        ...
                        <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-xl hover:bg-blue-700 transition flex items-center justify-center gap-3 shadow-xl shadow-blue-200 uppercase tracking-widest text-white">
                          {isLoading ? <Loader2 className="animate-spin" /> : "Talebi WhatsApp'a İlet"}
                        </button>
                    </form>
                ) : (
                    <div className="text-center py-10 animate-in zoom-in text-slate-900">
                        ...
                        <h3 className="text-2xl font-black mb-2 uppercase tracking-tighter text-slate-900">Servis Kaydınız Hazır!</h3>
                        <p className="text-slate-500 mb-8 font-medium">İzmir yerinde iPhone tamir talebiniz başarıyla alındı. Lütfen WhatsApp ekranında mesajı gönderin. Teknik ekibimiz <b className="text-blue-600">{phoneNumber}</b> üzerinden sizi arayacaktır.</p>
